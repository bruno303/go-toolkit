...
func TestFilter(t *testing.T) {
	cases := []struct {
		...
		{[]string{"a", "b", "c"}, func(s string) bool { return s == "a" }, []string{"a"}},
		{[]string{"a", "b", "c"}, func(s string) bool { return s == "d" }, []string{}},
		{[]string{}, func(s string) bool { return s == "d" }, []string{}},
	}

	for _, c := range cases {
		result := Filter(c.slice, c.f)
		if len(result) != len(c.expected) {
			t.Errorf("Expected '%d', got %d", len(c.expected), len(result))
		}
		for i := range result {
...
func TestRemove(t *testing.T) {
	cases := []struct {
		...
		{[]string{"a", "b", "c"}, func(s string) bool { return s == "a" }, []string{"b", "c"}},
		{[]string{"a", "b", "c"}, func(s string) bool { return s == "d" }, []string{"a", "b", "c"}},
		{[]string{}, func(s string) bool { return s == "d" }, []string{}},
	}

	for _, c := range cases {
		result := Remove(c.slice, c.f)
		if len(result) != len(c.expected) {
			t.Errorf("Expected '%d', got %d", len(c.expected), len(result))
		}
		for i := range result {
